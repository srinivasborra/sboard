    ...
    function encodeTags (html) {
        ...
        switch (typeof html) {
            ...
            case 'string':
                ...
                break;
            case 'object':
            	//Create new obj to ensure obj sent is not modified
            	var htmlObj = Array.isArray(html) ? $.extend(true, [], html) : $.extend(true, {}, html)
                for (var a in htmlObj) htmlObj[a] = this.encodeTags(htmlObj[a]);
            	return htmlObj;
                break;
        }
        return html;
    ...
    $.fn.w2marker = function (str) {
        if (str === '' || str == null) { // remove marker
            return $(this).each(function (index, el) {
                el.innerHTML = el.innerHTML.replace(/\<output class=\"w2ui\-marker\"\>(.*)\<\/output\>/ig, '$1'); // unmark
            });
        } else { // add marker
            return $(this).each(function (index, el) {
                ...
                function replaceValue(matched) { // mark new
                    return '<output class="w2ui-marker">' + matched + '</output>';
                }
            });
        }
    };
        ...
        return $(this).each(function (index, el) {
            ...
            function checkIfMoved(skipTransition) {
                ...
                if ($tags.data('position') !== posLeft + 'x' + posTop && skipTransition !== true) {
                    $tags.css(cssPrefix({ 'transition': '.2s' })).css({
                        left: posLeft + 'px',
                        top : posTop + 'px'
    ...
    w2grid.prototype = {
        ...
        getColumn: function (field, returnIndex) {
            if (arguments.length == 0) {
                var tmp = [];
                for (var i = 0; i < this.columns.length; i++) tmp.push(this.columns[i].field);
                return tmp;
            }
                    ...
                    for (var s in this.searchData) {
                        ...
                        switch (sdata.operator) {
                            case 'is':
                                ...
                                if (search.type == 'date') {
                                    var val1 = w2utils.formatDate(obj.parseField(rec, search.field + '_'), 'yyyy-mm-dd');
                                    var val2 = w2utils.formatDate(val2, 'yyyy-mm-dd');
                                    if (val1 == val2) fl++;
                                }
                                if (search.type == 'time') {
                                    var val1 = w2utils.formatTime(obj.parseField(rec, search.field + '_'), 'h24:mi');
                            ...
                            case 'between':
                                ...
                                if (search.type == 'date') {
                                    var val1 = obj.parseField(rec, search.field + '_');
                    ...
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            for (var s in this.searches) {
                                ...
                                if (['int', 'float', 'money', 'currency', 'percent'].indexOf(search.type) != -1 && String($.trim(value)).split('-').length == 2) {
                                ...
                                if (['list', 'enum'].indexOf(search.type) != -1) {
                                    ...
                                    if (new_values.length > 0) {
                                        var tmp = {
                                            field    : search.field,
                                            type     : search.type,
                                            operator : (new_values.length == 1 ? 'is' : 'in'),
                                            value    : (new_values.length == 1 ? new_values[0] : new_values)
                                        };
                                        searchData.push(tmp);
                                    }
                                }
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                html += '<tr '+ (this.isIOS ? 'onTouchStart' : 'onClick') +'="w2ui[\''+ this.name +'\'].initAllField(\''+ search.field +'\'); '+
                        '       event.stopPropagation(); $(\'#grid_'+ grid.name +'_search_all\').w2overlay({ name: \'searchFields-'+ grid.name +'\' });">'+
        ...
        initAllField: function (field, value) {
            ...
            if (field == 'all') {
                ...
                // look up value
                var tmp_val = value;
                if (st == 'list' && value != null) {
                    var items = w2obj.field.prototype.normMenu(search.options.items)
                    for (var i = 0; i < items.length; i++) {
                        if (items[i].id == value) {
                            tmp_val = items[i];
                            break;
                ...
                el.w2field(st, $.extend({}, search.options, { suffix: '', autoFormat: false, selected: tmp_val }));
                if (['list', 'enum', 'date'].indexOf(search.type) != -1) {
                    this.last.search = '';
                    this.last.item   = '';
                    el.val('');
                }
            ...
            $().w2overlay({ name: 'searchFields-' + this.name });
        },
        ...
        reload: function (callBack) {
            ...
            grid.selectionSave();
            if (url) {
                // this.clear(true); // do not clear as it clears selection
                this.last.scrollTop  = 0;
                this.last.scrollLeft = 0;
                this.last.xhr_offset = 0;   // need this for reload button to work on remote data set
                this.request('get-records', {}, null, function () {
        ...
        scrollIntoView: function (ind, instant) {
            var buffered = this.records.length;
            if (this.searchData.length != 0 && !this.url) buffered = this.last.searchIds.length;
            ...
            if (records[0]) {
                ...
                if (ind == t1) records.animate({ 'scrollTop': records.scrollTop() - records.height() / 1.3 }, instant ? 0 : 250, 'linear');
                if (ind == t2) records.animate({ 'scrollTop': records.scrollTop() + records.height() / 1.3 }, instant ? 0 : 250, 'linear');
                if (ind < t1 || ind > t2) records.animate({ 'scrollTop': (ind - 1) * this.recordHeight }, instant ? 0 : 250, 'linear');
            }
        ...
        render: function (box) {
            ...
            if (this.last.field && this.last.field != 'all') {
                ...
                setTimeout(function () { obj.initAllField(obj.last.field, (sd.length == 1 ? sd[0].value : null)); }, 150);
            }
            // init footer
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                if (this.show.toolbarSearch) {
                    var html =
                        ...
                        '                if (fld && fld.type == \'list\' && typeof sel.id == \'undefined\') {'+
                        '                   grid.searchReset();'+
                        '                } else {'+
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        case 'w2ui-reload':
                            ...
                            // obj.reset(); // do not reset as it clears selection
                            obj.reload();
                            obj.trigger($.extend(eventData2, { phase: 'after' }));
                            break;
        ...
        getCellHTML: function (ind, col_ind, summary) {
            ...
            if (typeof col.render != 'undefined') {
                ...
                if (typeof col.render == 'string') {
                    ...
                    if (tmp[0] == 'datetime') {
                        var title = (w2utils.isInt(data) ? (new Date(data)).toUTCString() : data);
                        if (typeof tmp[1] == 'undefined' || tmp[1] == '') tmp[1] = w2utils.settings.date_display + ',|' + w2utils.settings.time_format;
                        data = '<div title="' + title +'">' + prefix + w2utils.formatDateTime(data, tmp[1]) + suffix + '</div>';
                    }
                    if (tmp[0] == 'age') {
                        var title = (w2utils.isInt(data) ? (new Date(data)).toUTCString() : data);
                        data = '<div title="' + title +'">' + prefix + w2utils.age(data) + suffix + '</div>';
                    }
                    if (tmp[0] == 'toggle') {
    ...
    w2popup = {
        ...
        open: function (options) {
            ...
            if ($('#w2ui-popup').length == 0) {
                ...
                // then content
                var msg = '<div class="w2ui-msg-title" style="'+ (options.title == '' ? 'display: none' : '') +'">' + btn + options.title + '</div>'+
                ...
                // parse rel=*
                var parts = $('#w2ui-popup .w2ui-box2 > .w2ui-msg-body');
                if (parts.find('div[rel=title], div[rel=body], div[rel=buttons]').length > 0) {
                    // title
                    var tmp = parts.find('div[rel=title]');
                    if (tmp.length > 0) { options['title'] = tmp.html(); tmp.remove(); }
                    ...
                    if (tmp.length > 0) options['body'] = tmp.html(); else options['body'] = parts.html();
                    // set proper body
                    body = $('#w2ui-popup .w2ui-box2 > .w2ui-msg-body').html(options.body);
                }
            ...
            function mvMove(evnt) {
                ...
                $('#w2ui-popup').css({
                    ...
                    '-webkit-transform': 'translate('+ tmp.div_x +'px, '+ tmp.div_y +'px)',
                    '-moz-transition': 'none',
        ...
        message: function (options) {
            ...
            if ($.trim(options.html) == '' && $.trim(options.body) == '' && $.trim(options.buttons) == '') {
                ...
                if ($.trim(options.body) != '' || $.trim(options.buttons) != '') {
                    options.html = '<div style="position: absolute; top: 0px; bottom: 45px; left: 0px; right: 0px;overflow: auto;">'+ options.body +'</div>'+
                        '<div style="position: absolute; height: 45px; bottom: 0px; left: 0px; right: 0px; border-top: 1px solid #e5e5e5;text-align: center;padding: 8px;">'+ options.buttons +'</div>';
                }
        ...
        resizeMessages: function () {
            ...
            $('#w2ui-popup .w2ui-popup-message').each(function () {
                ...
                if (moptions.originalWidth < 0) moptions.width = pWidth + moptions.originalWidth * 2;
                $(this).css({
                ...
                if (it.type === 'drop' || it.type === 'menu') {
                    if (it.checked) {
                        ...
                        setTimeout(function () {
                            ...
                            if (it.type === 'menu') {
                                el.w2menu(it.items, $.extend({ name: obj.name, left: left, top: 3 }, it.overlay, {
    ...
    w2sidebar.prototype = {
        ...
        refresh: function (id) {
            ...
            function getNodeHTML(nd) {
                ...
                //Add Hint
                var textTxt = $('<div/>').html(nd.text).text();
                var hint = nd.hint || textTxt;
                if (nd.group) {
                    html =
                        '<div  title="' + hint + '" class="w2ui-node-group"  id="node_'+ nd.id +'"'+
                    ...
                    html =
                    '<div  title="' + hint + '" class="w2ui-node '+ (nd.selected ? 'w2ui-selected' : '') +' '+ (nd.disabled ? 'w2ui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
    ...
    */
    ...
    w2field.prototype = {
        ...
        refresh: function () {
            ...
            if (['list'].indexOf(this.type) != -1) {
                ...
                setTimeout(function () {
                    ...
                    hawk.utils.decodeData(selected); // Need to decode list so that the characters <,>,& are displayed as they were entered or else they will be displayed as &lt; &gt; &amp;
                    // focus helpder
            ...
            if (['enum', 'file'].indexOf(this.type) != -1) {
                ...
                if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) {
                    setTimeout(function () {
                        div[0].scrollTop = 0; // if read only - scroll to top
                        div.addClass('w2ui-readonly')
                            .find('li').css('opacity', '0.8')
                            .parents('ul').find('.w2ui-list-remove').hide();
                        div.find('li.nomouse').hide()
                    ...
                    setTimeout(function () {
                        div.removeClass('w2ui-readonly')
                            ...
                            .parents('ul').find('.w2ui-list-remove').show();
                        div.find('li.nomouse').show()
                ...
                if (html != '') {
                    ul.prepend(html);
                            ...
                            && (div.find('input').length == 0 || $.trim(div.find('input').val()) == '')) {
                    var style =
                ...
                // ITEMS events
                div.off('scroll').on('scroll', function (event) { if (typeof obj.options.onScroll == 'function') obj.options.onScroll(event); })
                    .find('li')
                    .data('mouse', 'out')
                    .on('click', function (event) {
        ...
        resize: function () {
            ...
            if (obj.tmp.current_width == new_width) return;
        ...
        */
        ...
        addFocus: function () {
            ...
            var html =
                '<div class="w2ui-field-helper">'+
                '    <span style="position: relative; top: 4px;"><div class="w2ui-icon icon-search" style="opacity: 0"></div></span>'+
